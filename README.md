...
在视频上叠加双语字幕,复用现有翻译引擎(不另起服务)。

- **YouTube 优先**:解析页面字幕轨(timedtext),优先人工字幕、回落机翻;基础分句优化(碎句按标点/间隔合并)。
- **通用 HTML5 兜底**:其它用 `<video>+<track>` 的站点,cuechange 跟随。
- **整轨批量翻译**:走 `%%` 批量 + LRU 缓存,逐句回填;边播边译,正在显示的句子先出译文。
- **双语 / 仅译文**两种模式;options / popup 可开关。

字幕文本仅经你配置的引擎(内置 Google 或你的自定义端点),不收集、不上传。

> 字幕轨抓取:YouTube timedtext 接口需授权令牌,扩展经 background 进程代取(仅 `youtube.com/api/timedtext`,不代理其它请求)。

## 🖼️ 图片翻译(右键·图相关文本,不含 OCR)

在图片上**右键 → 翻译图片文字信息**:收集图片的 `alt` / `title` / `aria-label` / `figcaption` / 相邻说明文本,经引擎翻译后在图片旁弹气泡。options 可关。

**能力边界**:不含**像素级 OCR**(图中文字识别需视觉模型,成本高、隐私另算,本轮不做)。无文字信息的图片会友好提示,不静默失败。
